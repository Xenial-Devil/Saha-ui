// This file re-exports all component examples for the showcase
// The examples folder is excluded from the build via vite.config.ts

// Color System
export { default as ColorPalette } from "./ColorPalette";

// Basic Components
export { ButtonExample } from "./ButtonExample";
export { ButtonGroupExample } from "./ButtonGroupExample";
export { BadgeExample } from "./BadgeExample";
export { ChipExample } from "./ChipExample";
export { LinkExample } from "./LinkExample";
export { SeparatorExample } from "./SeparatorExample";
export { default as KbdExample } from "./KbdExample";

// Layout Components
export { ContainerExample } from "./ContainerExample";
export { StackExample } from "./StackExample";
export { GridExample } from "./GridExample";
export { SectionExample } from "./SectionExample";
export { AspectRatioExample } from "./AspectRatioExample";

// Card & Content
export { CardExample } from "./CardExample";
export { AvatarExample } from "./AvatarExample";
export { ImageExample } from "./ImageExample";
export { EmptyExample } from "./EmptyExample";

// Navigation
export { BreadcrumbExample } from "./BreadcrumbExample";
export { default as NavigationMenuExample } from "./NavigationMenuExample";
export { default as MenubarExample } from "./MenubarExample";
export { PaginationExample } from "./PaginationExample";
export { TabExample } from "./TabExample";
export { StepsExample } from "./StepsExample";

// Data Display
export { TableExample } from "./TableExample";
export { DataTableExample } from "./DataTableExample";
export { ListExample } from "./ListExample";
export { TreeExample } from "./TreeExample";
export { TimelineExample } from "./TimelineExample";
export { AccordionExample } from "./AccordionExample";
export { default as CollapsibleExample } from "./CollapsibleExample";

// Form Components
export { default as FormExample } from "./FormExample";
export { InputExample } from "./InputExample";
export { default as TextAreaExample } from "./TextAreaExample";
export { SelectExample } from "./SelectExample";
export { AutocompleteExample } from "./AutocompleteExample";
export { default as ComboboxExample } from "./ComboboxExample";
export { default as NativeSelectExample } from "./NativeSelectExample";
export { CheckboxExample } from "./CheckboxExample";
export { CheckboxGroupExample } from "./CheckboxGroupExample";
export { RadioExample } from "./RadioExample";
export { RadioAdvancedExample } from "./RadioAdvancedExample";
export { SwitchExample } from "./SwitchExample";
export { ToggleExample } from "./ToggleExample";
export { default as ToggleGroupExample } from "./ToggleGroupExample";
export { SliderExample } from "./SliderExample";
export { RatingExample } from "./RatingExample";
export { DatePickerExample } from "./DatePickerExample";
export { CalendarExample } from "./CalendarExample";
export { default as InputOTPExample } from "./InputOTPExample";
export { FieldExample } from "./FieldExample";
export { default as LabelExample } from "./LabelExample";
export { default as UploadExample } from "./UploadExample";
export { TagExample } from "./TagExample";
export { TagInputExample } from "./TagInputExample";

// Feedback Components
export { AlertExample } from "./AlertExample";
export { ToastExample } from "./ToastExample";
export { SonnerExample } from "./SonnerExample";
export { default as ProgressExample } from "./ProgressExample";
export { default as SpinnerExample } from "./SpinnerExample";
export { SkeletonExample } from "./SkeletonExample";

// Overlay Components
export { default as DialogExample } from "./DialogExample";
export { DrawerExample } from "./DrawerExample";
export { default as PopoverExample } from "./PopoverExample";
export { TooltipExample } from "./TooltipExample";
export { default as HoverCardExample } from "./HoverCardExample";
export { DropdownExample } from "./DropdownExample";
export { default as ContextMenuExample } from "./ContextMenuExample";
export { default as CommandExample } from "./CommandExample";

// Media Components
export { CarouselExample } from "./CarouselExample";
export { PlayButtonExample } from "./PlayButtonExample";

// Utility Components
export { default as ScrollAreaExample } from "./ScrollAreaExample";
export { default as ResizableExample } from "./ResizableExample";
export { ItemExample } from "./ItemExample";
export { FloatingActionButtonExample } from "./FloatingActionButtonExample";

// Typography
export { TypographyExample } from "./TypographyExample";

// Advanced Components
export { default as TextEditorExample } from "./TextEditorExample";
export { default as CodeEditorFrameworkExamples } from "./CodeEditorFrameworkExamples";
export { default as ChartExamples } from "./ChartExamples";

// Composition Pattern
export { default as AsChildExamples } from "./AsChildExamples";

// Aggregate component that includes all examples
export { AllComponentExamples } from "./AllComponentExamples";
