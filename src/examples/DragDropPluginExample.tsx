...
export const PluginExample: React.FC = () => {
  ...
  const loggingPlugin: DragDropPlugin = {
    ...
    onDrop: (event) => {
      addLog(`🔴 Drag ended / Dropped in: ${event.containerId}`);
    },
  };
  ...
  const validationPlugin: DragDropPlugin = {
    ...
    validateDrop: (event) => {
      ...
      const targetContainer =
        (event as any).targetContainerId || (event as any).containerId;
      if (targetContainer === "container-2" && event.item.id === "1") {
            ...
            >
              {(item, index) => (
                <DraggableItem
                  ...
                  index={index}
                  item={item}
                  className="p-3 mb-2 bg-white border border-blue-200 rounded-lg shadow-sm hover:shadow-md transition-shadow"
                >
                  <div className="flex items-center gap-2">
                    <DragHandle className="text-gray-400">☰</DragHandle>
            ...
            >
              {(item, index) => (
                <DraggableItem
                  ...
                  index={index}
                  item={item}
                  className="p-3 mb-2 bg-white border border-green-200 rounded-lg shadow-sm hover:shadow-md transition-shadow"
                >
                  <div className="flex items-center gap-2">
                    <DragHandle className="text-gray-400">☰</DragHandle>
                    <span>{item.content}</span>
