...
const tests = [
  ...
  },
  {
    ...
    expect: (result) => {
      const text = JSON.stringify(result);
      return (
        text.includes("recentComponents") ||
...
async function runTests() {
  ...
  try {
    ...
    if (failed === 0) {
      log(
        "\n🎉 All tests passed! Dynamic features are working correctly.",
        "green",
      );
    } else {
      log(
...
function showFeatures() {
  ...
  log(
    '  "Components for a dashboard?" → Recommends Card, Tabs, etc. ✓',
    "yellow",
  );
